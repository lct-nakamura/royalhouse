...
* ・メールアドレスのname属性の値が「Email」ではない場合、以下必須設定箇所の「$Emailcampaign」の値も変更下さい。
...
// する場合は、メール入力欄のname属性の値を「$Emailcampaign」で指定した値にしてください。
...
$require = array('お名前','emailcampaign');
...
// 送る場合は、フォーム側のメール入力欄のname属性の値が上記「$Emailcampaign」で指定した値と同じである必要があります
$remail = 1;
...
◇ご注意    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
このメールは申し込みフォームよりお問い合わせいただいた方へ自動送信しております。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
━━━━━━□■□　申し込み内容　□■□━━━━━━━━━━━━━━━━━━━━━
TEXT;
...
//※デフォルトは「する」。特に理由がなければ変更しないで下さい。メール入力欄のname属性の値が上記「$Emailcampaign」で指定した値である必要があります。
$mail_check = 1;
...
//配列の形「name="○○[]"」の場合には必ず後ろの[]を取ったものを指定して下さい。
$hankaku_array = array('電話番号', 'emailcampaign');
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Emailcampaign) $post_mail = h($val);
		if($key == $Emailcampaign && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
