...
* ・メールアドレスのname属性の値が「Email」ではない場合、以下必須設定箇所の「$Emailentry」の値も変更下さい。
...
// する場合は、メール入力欄のname属性の値を「$Emailentry」で指定した値にしてください。
...
// 送る場合は、フォーム側のメール入力欄のname属性の値が上記「$Emailentry」で指定した値と同じである必要があります
$remail = 1;

//自動返信メールの送信者欄に表示される名前　※あなたの名前や会社名など（もし自動返信メールの送信者名が文字化けする場合ここは空にしてください）
$refrom_name = "ロイヤルハウス キャンペーン事務局";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "【1/23(月)～家づくりWEBセミナー開催！】セミナー視聴URL・アンケートのご案内";
...
お世話になっております。
ロイヤルハウス キャンペーン事務局でございます。

この度は、弊社開催の「間取り新発見！5つのトレンドセミナー」に
お申込みいただき誠にありがとうございます。


セミナーへの参加方法を下記のとおりご案内します。
――――――――――――――――――――――――――
　「間取り新発見！5つのトレンド」セミナーの詳細
――――――――――――――――――――――――――
■開催　：1月23日(月)10時～2月15日(水)17時

■会場　：YouTubeオンデマンド配信

■内容　：イマドキの5つのトレンドから、
　　　　　快適な空間作りのアイデアを実例とともにご紹介♪

■ご登録者様 限定公開セミナーURL

　　下記URLをクリックし、セミナーへご参加ください。
　　 https://youtu.be/-Ku_6_tBRXs
　
　※スマートフォンでご視聴予定の方はお手数ですが、
　　事前にYoutubeへのアクセスもしくは、
　　YouTubeアプリのインストールをお願いいたします。　
　
　＊セミナー所要時間は30分程度です。
　＊視聴期間内のお好きな日にち・時間帯に視聴可能です。
　　該当URLをクリックしご視聴ください。

また、セミナーに関してのアンケートを行っております。
視聴後、下記URLへアクセスしていただきご記入ください。

■アンケートURL
　　https://forms.gle/WR4jRRGCywjnvb1D9
　（所要時間約3分）
...
【アンケート記入及び2023年3月末までにご契約いただいた方】には
参加・成約特典としてシャワーヘッド「ミラブル」をプレゼントしております♪

その他、セミナーのご意見・感想などもお待ちしております。
...
みなさまのセミナーへのご参加を心よりお待ちしております。
セミナーに関して、ご質問等がございましたら
お気軽に事務局までご連絡ください。

━━━━━━━━━━━━━━━━━━━━━━━━━━━
▼▼▼お問い合わせ先▼▼▼

ロイヤルハウスキャンペーン事務局：mail@royal-house.co.jp

【運営元：ロイヤルハウス株式会社】
住所：〒460-0002
愛知県名古屋市中区丸の内3丁目20番3号BPRプレイス久屋大通4階
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
//※デフォルトは「する」。特に理由がなければ変更しないで下さい。メール入力欄のname属性の値が上記「$Emailentry」で指定した値である必要があります。
$mail_check = 1;
...
//配列の形「name="○○[]"」の場合には必ず後ろの[]を取ったものを指定して下さい。
$hankaku_array = array('郵便番号', 'emailentry');
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Emailentry) $post_mail = h($val);
		if($key == $Emailentry && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
